...
# from openrlhf.utils.utils import enable_pytorch_expandable_segments
...
class BasePPORole(DistributedTorchRayActor):
    def _setup_distributed(self, strategy: DeepspeedStrategy):
        ...
        # set with_max_memory_fraction argument if need.
...
class RewardModelRayActor(BasePPORole):
    ...
    def forward(
        self, sequences: torch.LongTensor, attention_mask: Optional[torch.Tensor] = None, packed_seq_lens=None, visual_inputs: Optional[dict] = None,
    ) -> torch.Tensor:
        device = torch.cuda.current_device()
